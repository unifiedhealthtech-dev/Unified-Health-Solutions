...
export const useSocketNotifications = (onNewNotification) => {
  // ✅ Memoize user shape for referential stability — critical!
  const user = useSelector(state => {
    const u = state.auth.user;
    if (!u) return null;
    return {
      role: u.role,
      id: u.distributor_id || u.retailer_id,
    };
  }, (prev, next) => {
    // ✅ Only recompute if values actually change
  ...
  }, [user, onNewNotification]); // ✅ Safe — user is memoized, onNewNotification should be useCallback’d
};