...
export const getStockItems = async (req, res) => {
  try {
    ...
    // 2. Get all categories from pharma's stock
...
export const updateStock = async (req, res) => {
  try {
    ...
    const item = await DistributorStockItem.findOne({  // ✅ renamed to 'item'
...
export const deleteStock = async (req, res) => {
  try {
    ...
    const item = await DistributorStockItem.findOne({
      where: { stock_id: stockId, distributor_id: distributor_id }
    });
...
export const getStockDetails = async (req, res) => {
  try {
    ...
    const stockItems = await DistributorStockItem.findAll({
      include: [{ model: Product, as: 'Product' }],
      where: { product_code: productCode, distributor_id: distributor_id },
...
export const getProductBatches = async (req, res) => {
  try {
    ...
    const batches = await DistributorStockItem.findAll({
      where: { product_code, distributor_id: distributor_id },
      attributes: [
        'stock_id',
        'batch_number',
        'manufacturing_date',
